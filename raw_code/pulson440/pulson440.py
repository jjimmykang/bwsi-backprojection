...
class PulsON440:
    ...
    def request_health(self):
        '''Requests status from the radar

        returns True if radar is healthy, and False if not

        '''
        # Check if radar is connected
        if self.connected:
            self.logger.info('Radar is connected! Checking health...')
            # Create status request and send
            payload = {}
            ...
            try:
                ...
                raise RuntimeError("Failed to fetch status of radar!")

            status = payload['status']
            bit_test_result = payload['bit_test_result']
            if status == 0:
